...
def ancient_scene_build():
    ...
    # Import the actual face GLB instead of monkey
    face_path = r"e:\blender master\Copilot3D-f8972524-c93f-4559-9872-1f6dd8511979.glb"
    if os.path.exists(face_path):
        bpy.ops.import_scene.gltf(filepath=face_path)
        # Select all imported meshes and scale them
        imported_objs = [obj for obj in bpy.context.selected_objects if obj.type == 'MESH']
        for obj in imported_objs:
            obj.scale = (15, 15, 15) # Scale up massively
            obj.location = (0, 8, 0)
            obj.rotation_euler = (math.radians(90), 0, 0)
            # Replace materials with moss_mat
            obj.data.materials.clear()
            obj.data.materials.append(moss_mat)
    else:
        # Fallback to monkey if not found
    ...
    # Columns framing the face exactly like the reference
    # Reference shows two prominent pillars bridging over the face
    bpy.ops.mesh.primitive_cylinder_add(radius=0.8, depth=1, location=(-6, 12, 6))
    col1 = bpy.context.active_object
    col1.scale = (1, 1, 12)
    col1.location.z = 6
    col1.data.materials.append(moss_mat)
    
    bpy.ops.mesh.primitive_cylinder_add(radius=0.8, depth=1, location=(6, 12, 6))
    col2 = bpy.context.active_object
    col2.scale = (1, 1, 12)
    col2.location.z = 6
    col2.data.materials.append(moss_mat)

    # Bridge between pillars
    bpy.ops.mesh.primitive_cube_add(location=(0, 12, 12))
    bridge = bpy.context.active_object
    bridge.scale = (7, 1, 0.5)
    bridge.data.materials.append(moss_mat)
    ...
    # Camera
    bpy.ops.object.camera_add(location=(0, -8, 2.0), rotation=(math.radians(85), 0, 0))
...
def human_element():
    # Human silhouette
    bpy.ops.mesh.primitive_cylinder_add(radius=0.25, depth=1.7, location=(1.0, -3.0, 1.15))
...
def polish_pass():
    # God ray
    bpy.ops.object.light_add(type='SPOT', location=(0, 6, 22))
    spot = bpy.context.active_object
    spot.rotation_euler = (0, math.radians(-10), 0)
    spot.data.energy = 8000000.0 
    spot.data.spot_size = math.radians(45)
    spot.data.spot_blend = 0.8
    spot.data.color = (0.8, 0.9, 1.0) # cooler moonlight/godray
    ...
    out_path = os.path.join(out_dir, "ancient_ruin_recreated.png")
